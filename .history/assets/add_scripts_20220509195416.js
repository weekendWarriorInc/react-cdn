...
function add_scripts(entry_script_url, entry_tag) {
    ...
    if (entry_tag) {
        ...
        let script = file_get_contents(entry_script_url)
        if (script) {
            if (find_imports(script).length) {
                replace_imports(script)
            }
            first_react_script.textContent = script
...
function replace_imports(text) {
    ...
    function rec(n_text) {
        ...
        if (match && match.length) {
            ...
            for (const item of match) {
                let url = item.match(/("\s*[\S]*\s*")|('\s*[\S]*\s*')/)[0].replace(/'|"/gm, '')
               
               let ond_text =file_get_contents(url)
               let new_match =find_imports(ond_text)
               let n_text =  ond_text.replace(item.trim())
                if (n_text&&new_match.length) {
                    rec(ond_text)
