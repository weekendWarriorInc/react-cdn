...
function replace_import_to_code(text) {
    let result = []
    let reg = /(import\s[\w\s,]*\sfrom\s['"\s*][\s\w\.\/-]*["'])|(import\s{[\w\s,]*}\sfrom\s['"\s*][\s\w\.\/-]*["'])/gm
    let match = text.match(reg)
    if (match && match.length) {
        ...
        for (const item of match) {
            ...
            if (src.length) {
                result.push(src[0].replace(/'|"/gm, ''))
                text.replace(match, file_get_contents(src))
                let n_text = file_get_contents(src)

                if (find_imports(n_text).length) {
                    replace_import_to_code(n_text)
                }
            }
        }
